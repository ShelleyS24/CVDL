...
def draw(img, corners, img_point, len):
    ...
    for i in range(len):
        img = cv2.line(
            img, tuple(img_point[2 * i]), tuple(img_point[2 * i + 1]), (0, 0, 255), 30
        )
    return img
...
def horizontallyClick(self, inWord):
    ...
    word = []
    text = inWord.upper()
    lib = os.path.join(self.loadAllFile, "Q2_db/alphabet_db_onboard.txt")
...
def verticallyClick(self, inWord):
    ...
    word = []
    text = inWord.upper()
    lib = os.path.join(self.loadAllFile, "Q2_db/alphabet_db_vertical.txt")
    fs = cv2.FileStorage(lib, cv2.FILE_STORAGE_READ)
